...
def on_message(ctx):
    ...
    if ctx.command == "!hello":
        ctx.reply("Greetings!")

    elif ctx.command == "!smile":
        ctx.add_reaction("\N{SLIGHTLY SMILING FACE}")

    elif ctx.command == "!download" or ctx.command == "!downloads":
        client.sendEmbed(
            ctx.channel_id, 7467907, "Download:",
            "[Download addon here](https://jaspervdijk63.github.io/downloads.html)")

    elif ctx.command == "!video" or ctx.command == "!videos":
        client.sendEmbed(
